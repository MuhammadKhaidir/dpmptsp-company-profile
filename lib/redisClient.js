...
//
// Env vars yang dicek (otomatis ke-set kalau kamu connect integrasi
// "Upstash Redis" lewat tab Storage/Marketplace di dashboard Vercel):
//   UPSTASH_REDIS_REST_URL   / UPSTASH_REDIS_REST_TOKEN   (nama "resmi" Upstash SDK)
//   KV_REST_API_URL          / KV_REST_API_TOKEN          (nama yang dipakai integrasi
//                                                           "Upstash for Redis" via Vercel
//                                                           Marketplace -- ini yang paling
//                                                           sering muncul di project baru)
//
// Kalau nama env var di project kamu ternyata beda lagi (misal dikasih prefix
...
function getRedis() {
    ...
    if (!url || !token) {
        throw new Error(
            ...
            'belum ditemukan di Environment Variables. ' +
