/**
 * FlyingPosters.js — versi VANILLA (bukan React)
 * Dikonversi dari komponen React "FlyingPosters" (ogl WebGL).
 * Class Media & Canvas persis sama seperti versi React-nya (memang sudah vanilla),
 * yang diganti cuma bagian mounting-nya (gak pake useRef/useEffect/JSX lagi).
 *
 * Load sebagai: <script type="module" src="js/FlyingPosters.js"></script>
 * Config lewat: window.FlyingPostersConfig = { items: [...], planeWidth, ... }
 * Target container: <div id="flyingPostersContainer"></div>
 */
...
/* =========================================================================
 * BAGIAN INI YANG MENGGANTIKAN "export default function FlyingPosters()"
 * React-nya. Gak pake ref/useEffect lagi — tinggal cari container di DOM,
 * bikin <canvas> di dalamnya, terus nge-mount Canvas class di atas.
 * ========================================================================= */

function mountFlyingPosters(container, options = {}) {
  const {
    items = [],
    planeWidth = 320,
    planeHeight = 320,
  ...
  container.classList.add('posters-container');
  ...
  // sama seperti useEffect kedua di versi React: cegah scroll halaman
  // ikut ke-trigger pas orang scroll di atas canvas si poster.
  const handleWheel = e => {
    e.preventDefault();
    instance.onWheel(e);
  };
...
function init() {
  const container = document.getElementById('flyingPostersContainer');
  if (!container) return;

  // ambil config dari window.FlyingPostersConfig yang di-set di index.html
  const config = window.FlyingPostersConfig || {};
  window.__flyingPostersInstance = mountFlyingPosters(container, config);
}
